...
from IDNN_B import Network
from IDNN_A import read_pytorch_return_reduction
...
def main():
    ...
    for new_idx, csv_item in enumerate(example_idx_list):
        ...
        # construct reduced network name
        if arguments.Config["model"]["onnx_path"] is not None:
            onnx_path = arguments.Config["model"]["onnx_path"]
            net_file = onnx_path.split("/")[-1]
        else:
            if arguments.Config["general"]["root_path"] is not None:
                onnx_path = os.path.join(arguments.Config["general"]["root_path"], onnx_path.strip())
            net_file = onnx_path.split("/")[-1]

        net_file = net_file.split(".")[0]
        rednet_name = "red_"+net_file+"_"+str(vnnlib_id)
        if arguments.Config["specification"]["epsilon"] is not None:
            rednet_name = rednet_name + "_" + str(arguments.Config["specification"]["epsilon"])
        rednet_name = rednet_name+str(".onnx")
        print(rednet_name)
        ...
        lbs, ubs = x_range.select(-1, 0), x_range.select(-1, 1)
        data_min = lbs.reshape(vnnlib_shape)
        data_max = ubs.reshape(vnnlib_shape)
        x = x_range.mean(-1).reshape(vnnlib_shape)  # only the shape of x is important.

        model_ori, reduce_time, bound_time = read_pytorch_return_reduction(input_model, data_min, data_max, propagation='crown')
        model_ori.eval()  

        tonnx.export(model_ori, x , rednet_name,\
            export_params=True,\
            opset_version=10,\

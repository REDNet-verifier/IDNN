...
def main():
    ...
    for new_idx, csv_item in enumerate(example_idx_list):
        ...
        csvpath = "alpha-beta-crown-IDNN.csv"
        ...
        if run_mode == 'single_vnnlib':
            ...
            with open(csvpath, 'a+', newline='') as write_obj:
                ...
                csv_writer.writerow([net_file, "idx: "+str(new_idx), "vnnlib ID: "+str(vnnlib_id), 'Result: '+str(verified_status), 'Time: '+str(time.time() - start_time)])
        else:
            cnt += 1
            ...
            with open(csvpath, 'a+', newline='') as write_obj:
                csv_writer = csv.writer(write_obj)
                csv_writer.writerow([net_file, "idx: "+str(new_idx), "vnnlib ID: "+str(vnnlib_id), 'Result: '+str(verified_status), 'Time: '+str(status_per_sample_list[-1][1])])
    if run_mode != 'single_vnnlib':
